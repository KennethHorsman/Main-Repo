...
def loadImages():
  ...
  # Extract the snake image used on the title screen and between rounds
  title = tk.PhotoImage()
  title.tk.call(title, 'copy', allImages, '-from', 0, 600, 800, 600+600, '-to', 0, 0)
  ...
  return background, title, gameover
...
def main():
  ...
  # Load all of the images used by the game
  background, title, gameover = loadImages()

  # Get the number of AI players from the user
  num_ai = getAICount(title)
  ...
  # Get ready to play!
  countdown(COUNTDOWN_DURATION, title)
  state = "playing"
  max_score = 0
...
###############################################################################
##
##  Do not modify the code that follows.
##
###############################################################################
